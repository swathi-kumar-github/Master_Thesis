...
\publishers{\normalsize
  \begin{minipage}[t]{.9\textwidth}
    \abstract
		Anomaly detectors commonly struggle to accurately identify defective products due to variations in recording conditions, such as lighting, shadow, and random placement. These conditions can result in the erroneous classification of normal products as anomalies (false positives). This challenge arises from training anomaly detection tools on a limited dataset of optimal images, which does not represent the full spectrum of real-world scenarios. This master's thesis focuses on generating and evaluating synthetic test images to enhance the performance of these anomaly detectors in industrial contexts. Generative models, including Cycle-Consistent Generative Adversarial Network(CycleGAN) was used to generate synthetic images by transforming optimal setup images to include bad camera influences and Stable Diffusion(img2img) to generate synthetic images directly from real images. The quality and similarity of these synthetic images were evaluated through various metrics to ensure their effectiveness. The performance of the anomaly detector was evaluated through different training approaches.The results demonstrated a significant reduction in false positives when trained with images (both real and synthetic) that included those camera influences.
  \end{minipage}
}
